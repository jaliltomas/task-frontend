    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex items-center gap-3">
        <div class="flex items-center gap-2">
          ...
          <input type="date" v-model="generationDate" class="form-input py-1.5 w-auto">
        </div>
        <button 
          @click="generateLists" 
          class="btn-primary"
...
const generationDate = ref(new Date().toISOString().split('T')[0])
...
async function generateLists() {
  ...
  try {
    const params = {}
    if (generationDate.value) {
      params.date = generationDate.value
    }
    const response = await axios.get(`${API_URL}/products/lists/generate`, { params })
    currentList.value = response.data
